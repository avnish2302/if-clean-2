...
app.post("/analyze-cleanliness", upload.single("image"), async (req, res) => {
    try {
        ...
        if (!parsed.valid) {                                                        // Stops processing early. Saves compute and avoid wrong logic
          parsed.alert = "none"
          return res.json(parsed)
        }

        parsed.cleanliness.confidence = Number(parsed.cleanliness.confidence) || 0  // Ensures number

        if (parsed.cleanliness.issues.length > 0) {                                 // if issues -> dirty
          parsed.cleanliness.status = "dirty"
        } else if (parsed.cleanliness.confidence < 0.3) {                           // low confidence -> uncertain
          parsed.cleanliness.status = "unknown"
        } else {                                                                    // No issues + decent confidence -> clean
